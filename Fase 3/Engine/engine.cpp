    ...
    for (XMLElement *elem = file->FirstChildElement(); elem != NULL; elem = elem->NextSiblingElement())
    {
        if (strcmp(elem->Name(), "transform") == 0)
        {
            ...
            if (rotate)
            {
                ...
                const XMLAttribute *time = rotate->FindAttribute("time");
                if (time)
                {
                    float time_r = atof(time->Value());
            ...
            if (translate)
            {
                const XMLAttribute *time = translate->FindAttribute("time");
                if (time)
                {
...
void drawForma(Forma *f)
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, f->getVBOID());
    glVertexPointer(3, GL_FLOAT, 0, 0); // dizer que cada vértice tem 3 floats
...
int main(int argc, char **argv)
{
    ...
    char caminho[1024];
    strcpy(caminho, "../test_files_phase_3/");
